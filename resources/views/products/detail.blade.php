                    ...
                    @if ($sizeColorStock->count() > 0)
                        <div x-data="{
                            ...
                            <div class="inline-flex">
                                <template x-init="[selectedColor = JSON.parse(JSON.parse(sizes[selectedSize][0].color)).name, selectedStockId = sizes[selectedSize][0].id ]" x-for="size in Object.keys(sizes)"
                                    :key="size">
                                    <span
                                        x-on:click="[
                                        selectedSize = size,
                                        selectedColor= JSON.parse(JSON.parse(sizes[selectedSize][0].color)).name,
                                        selectedStockId = sizes[selectedSize][0].id,
                                        ...
                                        :class="selectedSize === size ?
                                            'bg-yellow-600 border-2 border-white shadow-md shadow-black' : 'bg-gray-500'"
                                        x-text="size"></span>
                                </template>
                            </div>
                            <div class="my-3 font-bold">
                            ...
                            <div class="flex mt-3">
                                <template x-for="(color, index) in sizes[selectedSize]" :key="index">
                                    <div x-on:click="[selectedColor = JSON.parse(JSON.parse(color.color)).name, price = defaultPrice + JSON.parse(color.extra_plus)]"
                                        class="w-[40px] h-[40px] px-2 py-2 ml-2 cursor-pointer rounded-md"
                                        :class="selectedColor === JSON.parse(JSON.parse(color.color)).name ?
                                            'border-2 border-white shadow-md shadow-black' : 'border-1 border-back'"
                            ...
                            <div class="flex mt-3">
                                <template x-for="(color, index) in sizes[selectedSize]" :key="index">
                                    <img x-on:click="[selectedColor = JSON.parse(JSON.parse(color.color)).name, price = defaultPrice + JSON.parse(color.extra_plus)]"
                                        :class="selectedColor === JSON.parse(JSON.parse(color.color)).name ?
                                            'border-2 border-white shadow-md shadow-black' : 'border-1 border-back'"
                                        :src="color.image"
                                        class="ml-2 w-[65px] h-[65px] object-cover bg-cover cursor-pointer rounded-md transition-shadow"
                                        alt="color.name">
                                </template>
                            </div>
                            <div class="my-3 font-bold">
                                ...
                                <div class="flex">
                                    <span class="ml-2 text-sm font-semibold text-gray-500"
                                        x-text="`Option Name: ${selectedSize} , Color Name: ${selectedColor}, id: ${selectedStockId}`"></span>
                                </div>
                            </div>
                        </div>
                    @elseif ($sizeOnlyStock->count() > 0)
                        <div x-data="{
                            ...
                            <div class="my-3 font-bold">
                                ...
                                <div class="flex">
                                    <span class="ml-2 text-sm font-semibold text-gray-500"
                                        x-text="`Option Name: ${selectedSize}`"></span>
                                </div>
                            </div>
                        </div>
                    @elseif ($colorOnlyStock->count() > 0)
                        <div x-data="{
                            colors: {{ $colorOnlyStock }},
                            selectedColor: JSON.parse({{ $colorOnlyStock[0]->color }}).name
                        }" x-init="if ({{ $colorOnlyStock[0]->plus }}) price = defaultPrice + {{ $colorOnlyStock[0]->extra_plus }}">
                            <div class="my-3 font-bold">
                                Choose Colors
                            </div>
                            <div class="flex mt-3">
                            ...
                            <div class="flex mt-3">
                                <template x-for="(color, index) in colors" :key="index">
                                    <img x-show="color.image" x-on:click="[selectedColor = JSON.parse(JSON.parse(color.color)).name, price = defaultPrice + JSON.parse(color.extra_plus)]"
                                        :class="selectedColor === JSON.parse(JSON.parse(color.color)).name ?
                                            'border-2 border-white shadow-md shadow-black' : 'border-1 border-back'"
                                        :src="color.image"
                                        class="ml-2 w-[65px] h-[65px] object-cover bg-cover cursor-pointer rounded-md transition-shadow"
                            ...
                            <div class="my-3 font-bold">
                                ...
                                <div class="flex">
                                    <span class="ml-2 text-sm font-semibold text-gray-500"
                                        x-text="`Color Name: ${selectedColor}`"></span>
                                </div>
                            </div>
                        </div>
                    @endif
